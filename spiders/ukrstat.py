...
def import_stat(date) -> dict:
    ...
    data_list = data_string.find_all('font')
    if data_list == []:
    ...
    out_dict['_id'] = datetime.strptime(month_year, '%m%y').replace(day=1, hour=17, minute=0)
    out_dict['source'] = 'ukrstat'
    return out_dict


if __name__ == '__main__':
    print(json.dumps(import_stat(datetime.strptime('2009-06', '%Y-%m')), sort_keys=True, indent=4,
